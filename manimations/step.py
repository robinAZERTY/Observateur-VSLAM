...
class Step(MovingCameraScene):
    def construct(self):
      ...
      # Adding the probability distribution graph with mu and sigma
        ...
        label = MathTex(r"\hat{X}_{k-1}").next_to(graph, UP, buff=0.1).shift(RIGHT).set_color(BLUE)
        ...
        predictedLabel = MathTex(r"\hat{X}_{k}").next_to(predicted, UP, buff=0.1).set_color(BLUE)
        predictedmu_label = MathTex(str(mu)).next_to(axes.c2p(mu, 0), DOWN)
        predictedmu_dot = Dot(point=axes.c2p(mu, 0), color=BLUE)
        
        predGroup = VGroup(predicted, predictedLabel, predictedmu_dot, predictedmu_label)
        ...
        self.wait(2)
        # duration of transition is 3s
        self.play(FadeOut(commandeArrow),FadeOut(commande),Transform(graph,predicted), Transform(label,predictedLabel), Transform(mu_dot,predictedmu_dot), Transform(mu_label,predictedmu_label),run_time = 3)
    
        self.wait(2)
        self.play(FadeOut(axes),FadeOut(graph),FadeOut(label),FadeOut(predictedLabel),FadeOut(mu_dot),FadeOut(mu_label),FadeOut(pred))
        ...
        self.play(FadeOut(predState), FadeOut(a1), FadeOut(a2), FadeOut(prevState), FadeOut(mesure))
        self.play(Write(predGroup), Write(axes))
        
        # Adding the probability distribution of mesure
        mu = 22
        sigma = 1.5
        
        mesurment = axes.plot(lambda x: (1 / (sigma * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((x - mu) / sigma)**2), color=RED)
        mesurmentLabel = MathTex(r"Z_{k}").next_to(mesurment, UP, buff=0.1).shift(RIGHT).set_color(RED)
        mu_label = MathTex(str(mu)).next_to(axes.c2p(mu, 0), DOWN)
        mu_dot = Dot(point=axes.c2p(mu, 0), color=RED)
        MesurmentGroup = VGroup(mesurment, mesurmentLabel, mu_dot, mu_label)
        
        self.play(Write(MesurmentGroup))
        self.wait(2)
        ...
        updated = axes.plot(lambda x: (1 / (sigma * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((x - mu) / sigma)**2), color=BLUE)
        updatedLabel = MathTex(r"\hat{X}_{k+1}").next_to(updated, UP, buff=0.1).shift(RIGHT).set_color(BLUE)
        mu_label = MathTex(str(mu)).next_to(axes.c2p(mu, 0), DOWN)
        mu_dot = Dot(point=axes.c2p(mu, 0), color=BLUE)
        
        UpdatedGroup = VGroup(updated, updatedLabel, mu_dot, mu_label)
        
        self.play(Transform(predGroup, UpdatedGroup), FadeOut(MesurmentGroup), run_time=3)
        self.wait(2)
